...
const AdminCreateVoucherShopWide: React.FC = () => {
  ...
  const [currentValueInputs, setCurrentValueInputs] = useState<{ [key: string]: string }>({});
  ...
  const handleAddAttribute = () => {
    ...
    setCurrentValueInputs((prev) => ({ ...prev, [newAttribute.id]: "" }));
  };
  ...
  const handleValueInputChange = (id: string, value: string) => {
    setCurrentValueInputs((prev) => ({ ...prev, [id]: value }));
  };
  ...
  const handleValueKeyDown = (e: React.KeyboardEvent<HTMLInputElement>, id: string) => {
    if (e.key === "Enter") {
      ...
      const value = currentValueInputs[id]?.trim();
      if (value) {
        // Chỉ thêm giá trị vào attribute hiện tại
        setAttributes((prev) =>
          prev.map((attr) =>
            attr.id === id
        ...
        // Clear input để tiếp tục nhập
        setCurrentValueInputs((prev) => ({ ...prev, [id]: "" }));
      }
    }
  };

  const handleRemoveValue = (id: string, valueIndex: number) => {
    setAttributes((prev) =>
      prev.map((attr) =>
        attr.id === id
          ? {
  ...
  return (
    <div className="flex flex-col w-full max-w-full lg:max-w-[calc(100%-500px)] mx-auto mt-[10px] px-[16px] sm:px-[24px] lg:px-0">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-[16px] w-full mt-[8px]">
        {/* Basic Information Section */}
        <div className="bg-white border border-[#e7e7e7] rounded-[24px] p-[16px] sm:p-[24px] flex flex-col gap-[16px] w-full">
          {/* Title Section */}
          ...
          <div className="w-full flex justify-center">
            <div className="bg-[#e04d30] border border-white rounded-[12px] h-[52px] px-[12px] sm:px-[16px] flex items-center gap-[4px]">
                  ...
                  {formData.maxDiscountLimit === "limited" && (
                    <div>
                      <FormInput
                        ...
                        onChange={(e) =>
                          handleInputChange("maxDiscountValue", e.target.value)
                        }
                        containerClassName="h-[36px] w-full"
                      />
                    </div>
              ...
              <div className="flex-1 flex items-center min-w-0">
                ...
                >
                  ...
                  <span className="font-semibold text-[14px] text-[#e04d30] leading-[1.4]">
                    {isAttributesOpen ? "Ẩn thuộc tính" : "Mở thuộc tính"}
                  </span>
                </button>
              </div>
            </div>
      ...
      {/* Attributes Section */}
      {isAttributesOpen && (
        <div className="bg-white border border-[#e7e7e7] rounded-[24px] p-[16px] sm:p-[24px] flex flex-col gap-[16px] w-full mt-[16px]">
          <div className="flex flex-col gap-[8px]">
            <h2 className="font-bold text-[18px] sm:text-[20px] text-[#272424] leading-[normal] break-words">
              Thuộc tính
            </h2>
          ...
          <div className="flex flex-col gap-[12px]">
            {/* Header */}
            <div className="bg-[#f5f5f5] border border-[#e7e7e7] rounded-t-[12px] px-[16px] py-[12px] grid grid-cols-[1fr_2fr_auto] gap-[16px] items-center">
              <div className="font-semibold text-[14px] text-[#272424]">Tên thuộc tính</div>
              <div className="font-semibold text-[14px] text-[#272424]">Giá trị</div>
              <div className="w-[40px]"></div>
            </div>

            {/* Attributes Rows */}
            {attributes.length > 0 && (
              <div className="border-l border-r border-b border-[#e7e7e7] rounded-b-[12px] overflow-hidden">
                {attributes.map((attr, index) => (
                  <div
                    key={attr.id}
                    className={`grid grid-cols-[1fr_2fr_auto] gap-[16px] px-[16px] py-[12px] items-start ${
                      index !== attributes.length - 1 ? "border-b border-[#e7e7e7]" : ""
                    }`}
                  >
                    ...
                    <div className="flex items-center">
                      <input
                        ...
                        className="w-full h-[36px] px-[12px] border border-[#e7e7e7] rounded-[8px] text-[14px] text-[#272424] focus:outline-none focus:border-[#e04d30]"
                      />
                    </div>

                    {/* Values Input with Tags */}
                    <div className="flex items-start">
                      <div className="w-full min-h-[36px] px-[8px] py-[6px] border border-[#e7e7e7] rounded-[8px] flex flex-wrap gap-[6px] items-center focus-within:border-[#e04d30] bg-white">
                        {/* Tags */}
                        {attr.values.map((value, valueIndex) => (
                          <div
                            key={valueIndex}
                            className="inline-flex items-center gap-[4px] px-[8px] py-[4px] bg-[#e3f2fd] rounded-[16px] text-[13px] text-[#272424]"
                          >
                            <span>{value}</span>
                            <button
                              type="button"
                              onClick={() => handleRemoveValue(attr.id, valueIndex)}
                              className="flex items-center justify-center w-[16px] h-[16px] hover:bg-[#bbdefb] rounded-full transition-colors"
                            >
                              <Icon name="close" size={12} color="#272424" />
                            </button>
                          </div>
                        ))}
                        {/* Input Field */}
                        <input
                          type="text"
                          placeholder={attr.values.length === 0 ? "Nhập ký tự và ấn enter" : ""}
                          value={currentValueInputs[attr.id] || ""}
                          onChange={(e) => handleValueInputChange(attr.id, e.target.value)}
                          onKeyDown={(e) => handleValueKeyDown(e, attr.id)}
                          className="flex-1 min-w-[120px] h-[24px] border-none outline-none text-[13px] text-[#272424] placeholder:text-[#9e9e9e] bg-transparent"
                          autoComplete="off"
                        />
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-center pt-[6px]">
                      ...
                      >
                        <Icon name="trash" size={18} color="#e04d30" />
                      </button>
                    </div>
                  </div>
                ))}
            ...
            <div className="mt-[4px]">
              <button
                ...
                className="bg-white border border-[#e7e7e7] rounded-[8px] px-[12px] py-[8px] flex items-center gap-[8px] hover:bg-[#f5f5f5] transition-colors"
              >
                <Icon name="plus" size={16} color="#272424" />
