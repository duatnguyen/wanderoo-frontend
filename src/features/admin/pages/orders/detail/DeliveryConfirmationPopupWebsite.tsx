...
const DeliveryConfirmationPopupWebsite: React.FC<
  DeliveryConfirmationPopupWebsiteProps
> = ({ isOpen, onClose, onConfirm, orderData }) => {
  ...
  const [selectedShift, setSelectedShift] = useState<PickShiftItem | null>(
    null
  );
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <button
                ...
                disabled={isSubmitting}
                className={`group relative p-5 rounded-[16px] border-2 transition-all duration-200 text-left ${
                  selectedMethod === "self"
              ...
              >
                <div className="flex flex-col space-y-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <p
                        className={`font-montserrat font-semibold text-[14px] leading-tight transition-colors ${
                          selectedMethod === "self"
                            ? "text-[#1a71f6]"
                            : "text-gray-900"
                        }`}
                      ...
                      <div className="text-left">
                        ...
                        {selectedShift && (
                          <p className="font-montserrat font-medium text-[12px] text-gray-500">
                            {formatTime(selectedShift.from_time)} -{" "}
                            {formatTime(selectedShift.to_time)}
                          </p>
                        )}
                      </div>
                    </div>
                    <ChevronDown
                      className={`w-5 h-5 text-gray-400 transition-transform duration-200 ${showShiftDropdown ? "rotate-180" : ""}`}
                    />
                  </button>
                    ...
                    <div className="absolute top-full left-0 right-0 mt-2 bg-white border-2 border-gray-200 rounded-[14px] shadow-lg z-10 max-h-60 overflow-y-auto">
                      {pickShifts.map((shift) => (
                        <button
                          ...
                          className={`w-full p-4 flex items-center gap-3 hover:bg-gray-50 transition-colors first:rounded-t-[12px] last:rounded-b-[12px] ${
                        ...
                        >
                          ...
                          <div className="text-left">
                            <p
                              className={`font-montserrat font-semibold text-[13px] ${
                                selectedShift?.id === shift.id
                                  ? "text-orange-900"
                                  : "text-gray-900"
                              }`}
                            ...
                            <p className="font-montserrat font-medium text-[11px] text-gray-500">
                              {formatTime(shift.from_time)} -{" "}
                              {formatTime(shift.to_time)}
                            </p>
                          </div>
                          {selectedShift?.id === shift.id && (
                ...
                {[
                  {
                    value: "CHOTHUHANG",
                    label: "Cho thử hàng",
                    description: "Cho phép khách hàng thử hàng trước khi nhận",
                  },
                  {
                    value: "CHOXEMHANGKHONGTHU",
                    label: "Cho xem hàng không thử",
                    description: "Cho xem hàng nhưng không cho thử",
                  },
                  {
                    value: "KHONGCHOXEMHANG",
                    label: "Không cho xem hàng",
                    description: "Không cho xem hay thử hàng",
                  },
                ].map((option) => (
                  <label
                    key={option.value}
                  ...
                  <div>
                    ...
                    {selectedMethod === "pickup" && selectedShift && (
                      <p className="font-montserrat font-medium text-[12px] text-red-600 mt-1">
                        Ca lấy hàng: {selectedShift.title} (
                        {formatTime(selectedShift.from_time)} -{" "}
                        {formatTime(selectedShift.to_time)})
                      </p>
                    )}
