...
import { ChevronDown, ChevronUp, Wallet, CheckCircle, Clock, XCircle, Banknote, Building2, HelpCircle } from "lucide-react";
...
const PaymentInformationCollapsedView = memo<PaymentInformationCollapsedViewProps>(({
  isExpanded,
  onToggle,
  totalAmount,
  formatCurrency,
  ...
  return (
    <button
      ...
      aria-label={isExpanded ? "Thu gọn thông tin thanh toán" : "Mở rộng thông tin thanh toán"}
    >
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-3">
        ...
        <div className="flex items-center gap-2 flex-shrink-0 w-full sm:w-auto justify-between sm:justify-end">
          ...
          <div className="hidden sm:flex items-center gap-2">
            <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-md border ${paymentStatusInfo.badgeClassName}`}>
              <StatusIcon className={`w-3.5 h-3.5 ${paymentStatusInfo.iconClassName}`} />
              <span className={`font-montserrat font-semibold text-[10px] ${paymentStatusInfo.textClassName} whitespace-nowrap`}>
                {paymentStatusInfo.label}
              </span>
            </div>

            <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-md border ${paymentMethodInfo.badgeClassName}`}>
              <MethodIcon className={`w-3.5 h-3.5 ${paymentMethodInfo.iconClassName}`} />
              <span className={`font-montserrat font-semibold text-[10px] ${paymentMethodInfo.textClassName} whitespace-nowrap`}>
          ...
          <div className="flex sm:hidden items-center gap-1.5 flex-1">
            <div className={`flex items-center gap-1 px-2 py-1 rounded border ${paymentStatusInfo.badgeClassName} flex-1 justify-center`}>
              <StatusIcon className={`w-3 h-3 ${paymentStatusInfo.iconClassName}`} />
              <span className={`font-montserrat font-semibold text-[10px] ${paymentStatusInfo.textClassName} truncate`}>
                {paymentStatusInfo.label}
              </span>
            </div>
            <div className={`flex items-center gap-1 px-2 py-1 rounded border ${paymentMethodInfo.badgeClassName} flex-1 justify-center`}>
              <MethodIcon className={`w-3 h-3 ${paymentMethodInfo.iconClassName}`} />
              <span className={`font-montserrat font-semibold text-[10px] ${paymentMethodInfo.textClassName} truncate`}>
                {paymentMethodInfo.label}
              </span>
            </div>
          </div>
...
const PaymentInformationWebsite: React.FC<PaymentInformationWebsiteProps> = ({
  ...
  const paymentStatus = useMemo(() => orderData?.paymentStatus || "PENDING", [orderData?.paymentStatus]);
  const paymentMethod = useMemo(() => orderData?.method || "UNDEFINED", [orderData?.method]);
  ...
  const paymentData = useMemo(() => {
    ...
    return [
      ...
      { label: "Mã giảm giá của shop", amount: -discountAmount, showZero: discountAmount === 0 },
      {
  ...
  }, [orderData?.totalProductPrice, orderData?.shippingFee, orderData?.totalOrderPrice]);
  ...
  return (
    <div
      className={`bg-white border-2 border-gray-200 box-border relative rounded-lg w-full overflow-hidden min-w-0 shadow-sm transition-all duration-200 ${disabled ? "opacity-50 pointer-events-none" : ""
        }`}
    >
              ...
              {paymentData.slice(0, -1).map((item, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className={`font-montserrat font-semibold text-sm ${item.amount < 0 ? "text-green-600" : "text-blue-600"
                      }`}>
