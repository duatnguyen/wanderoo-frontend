...
const AdminCreateVoucherReturningCustomer: React.FC = () => {
  ...
  const parseReturningDescription = (value?: string | null) => {
    if (!value) {
      return {
        description: "",
        totalSpendingAmount: "",
      ...
      return {
        description:
          typeof parsed.description === "string" && parsed.description.length > 0
            ...
            : value,
        totalSpendingAmount:
          parsed.totalSpendingAmount != null ? String(parsed.totalSpendingAmount) : "",
        spendingDays: parsed.spendingDays != null ? String(parsed.spendingDays) : "",
      };
    } catch {
      return {
        description: value,
  ...
  const encodeReturningDescription = (form: VoucherFormData) => {
    const totalSpending =
      form.totalSpendingAmount && !Number.isNaN(Number(form.totalSpendingAmount))
        ? Number(form.totalSpendingAmount)
        : undefined;
    const spendingDaysValue =
      form.spendingDays && !Number.isNaN(Number(form.spendingDays))
        ? Number(form.spendingDays)
        : undefined;
    const baseDescription = form.description?.trim() ?? "";

    if (!totalSpending && !spendingDaysValue) {
      return baseDescription || undefined;
    }

    return JSON.stringify({
      description: baseDescription,
      totalSpendingAmount: totalSpending,
      spendingDays: spendingDaysValue,
    });
  };
  ...
  const mapDetailToFormData = (detail: AdminDiscountResponse): VoucherFormData => ({
    ...
    ...parseReturningDescription(detail.description),
  ...
  useEffect(() => {
    ...
    if (mode === "edit" && editData) {
      setFormData({
        ...
        totalSpendingAmount: editData.totalSpendingAmount ?? "",
        spendingDays: editData.spendingDays ?? "",
  ...
  const buildPayload = (): AdminDiscountCreateRequest => {
    ...
    return {
      ...
      quantity: usageLimit,
      status: "ENABLE",
      description: encodeReturningDescription(formData),
    };
  };
                ...
                <div className="flex-1 flex flex-col gap-[8px]">
                  ...
                  <div className="flex flex-wrap items-center gap-[4px] text-[14px] text-[#272424]">
                    <span className="font-medium">
                      Khách hàng từng mua sắm tại shop ≥
                    </span>
