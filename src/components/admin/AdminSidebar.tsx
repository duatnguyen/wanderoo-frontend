...
import { adminNavSections, adminFooterNav, type AdminNavItem, type AdminNavSection } from './adminNavData.tsx';
...
const AdminSidebar: React.FC<AdminSidebarProps> = ({ activePath }) => {
    ...
    const renderNavItem = (item: AdminNavItem, key: React.Key, isSubmenu: boolean = false) => {
        ...
        if (hasSubmenu) {
            ...
            isActive = item.activeMatch ? activePath.startsWith(item.activeMatch) : Boolean(item.defaultActive);
        }
        ...
        let itemClasses;
        if (isSubmenu) {
            itemClasses = `${baseClasses} ${submenuClasses} ${isActive ? submenuActiveClasses : submenuInactiveClasses}`;
        } else {
            itemClasses = `${baseClasses} ${isActive ? activeClasses : inactiveClasses} ${hasSubmenu ? 'cursor-pointer' : ''}`;
        }
        ...
        <aside className="w-[253px] shrink-0 bg-[#18345C] text-white">
            <div className="flex h-full flex-col px-[16px] pb-8 py-[32px]">
                <div className="flex flex-col items-center gap-4 text-center">
                        <img src={ShopLogo} alt="Shop Logo" className="h-[200px] w-full object-contain" />
                </div>
                        ...
                        return (
                            <div key={sectionKey} className={wrapperClasses}>
                                ...
                                <div className={listClasses}>
                                    {section.items.map((item: AdminNavItem, itemIndex: number) => renderNavItem(item, `${sectionKey}-${itemIndex}`))}
                                </div>
                            </div>
                        );
                    })}
                ...
                <div className="mt-8 border-t border-white/10 pt-6">
                    <div className="space-y-1.5">
                        {adminFooterNav.items.map((item: AdminNavItem, index: number) => renderNavItem(item, `footer-${index}`))}
                    </div>
                </div>
            </div>
        </aside>
